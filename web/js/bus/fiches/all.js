...
//-----FILTER ENQUETEUR----------------
  $('.enq_select').change(function(){
    var value = $(this).val();
    if( value!="-" )
     window.location.href = Routing.generate('bus_questionnaire_all_by_filter', { typeFilter: 'enqueteur', dataFilter: $(this).val(), table: $('.table_info').text(), mois: $('.mois_info').text() });
});
//-----FILTER LIGNE----------------
  $('.ligne_select').change(function(){
    var value = $(this).val();
    if( value!="-" )
      window.location.href = Routing.generate('bus_questionnaire_all_by_filter', { typeFilter: 'ligne', dataFilter: $(this).val(),
  ...
  $('.date_select').change(function(){
    var value = $(this).val();
    if( value!="-" )
      window.location.href = Routing.generate('bus_questionnaire_all_by_filter', { typeFilter: 'date', dataFilter: $(this).val(),
                         table: $('.table_info').text(), mois: $('.mois_info').text() });
  });
  //-----FILTER PA----------------
  $('.PA_select').change(function(){
    var value = $(this).val();
    if( value!="-" )
      window.location.href = Routing.generate('bus_questionnaire_all_by_filter', { typeFilter: 'arretMontee', dataFilter: $(this).val() });
  });
  ...
  $('.history').change(function(){
    window.location.href = Routing.generate('bus_questionnaire_all', { 'mois': $(this).val() });
  });
...
function exportMesures( )
{
  ...
  $.ajax({
      ...
      url : Routing.generate('bus_questionnaire_all_export', true),
      type : 'POST',
      async: false, 
      dataType : 'json',
      success : function(response, statut){
        window.location.href = base_url+'export_public/MESURES_BUS.xlsx';
...
function getInfosMesures( )
{
  ...
  $.ajax({
        ...
        url : Routing.generate('bus_questionnaire_quotas_mesures_valides', true),
    type : 'POST',
    async: false, 
    dataType : 'json',
    success : function(response, statut){
