...
$(document).ready(function () {
    ...
    $('.submit').click(function (e) {
        ...
        id_mesure = form.find('.id_mesure').val();
        ...
        editApi(input_file, id_mesure, photo_name, img, ligne, date);
    });

});

function remove(id_mesure, photo_name, elem) {
    var base_url = Routing.getBaseUrl().replace(/\w+\.php$/gi, '');
    $('.loader').append('<img src="' + base_url + '/images/gears.svg" class="svg-loader" />');
    var ligne = $('.ligne').text();
    var date = $('.date').text();
    removeApi(id_mesure, photo_name, elem, ligne, date);
}
...
function viewImg(ligne, date_format, photo_name) {
    var base_url = Routing.getBaseUrl().replace(/\w+\.php$/gi, '');
    window.open(base_url + "photos/bus/" + ligne + '_' + date_format + "/" + photo_name, "_blank", "directories=no,titlebar=no,toolbar=no,location=no,status=no,menubar=no, scrollbars=yes,resizable=yes,top=100,left=100,width=700,height=500");
}

function editApi(input_file, id_mesure, photo_name, img, ligne, date) {

    var data = new FormData();
    data.append('input_file', input_file);
    data.append('id_mesure', id_mesure);
    data.append('photo_name', photo_name);

    $.ajax({
        url: $('.url_photos_api').text() + "_BUS/update-photo-bus",
        type: "POST",
        data: data,
        cache: false,
        contentType: false,
...
function removeApi(id_mesure, photo_name, elem, ligne, date) {
    $.ajax({

        url: $('.url_photos_api').text() + "_BUS/remove-photo-bus",
        type: 'POST',
        async: false,
        data: "id_mesure=" + id_mesure + "&photo_name=" + photo_name,
        dataType: 'json',
