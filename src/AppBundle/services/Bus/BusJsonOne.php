...
class BusJsonOne
{
	...
	public function __construct( EntityManager $entityManager, $base_app )
	{
		$this->repository = $entityManager->getRepository('AppBundle:BusJson');
		$this->text = "";
		$this->result_generation_json = false;
		$this->fileLocation = realpath( dirname(__FILE__).$base_app ).'/JSON/bus/emet/';
	}
	...
	public function init( $id, $date )
	{
		...
		$id = 214;
		$this->list_grilles = $this->repository->listGrillesByOne( $id );
	...
	private function getJson()
	{
		for( $i=0; $i<sizeof( $this->list_grilles ); $i++ )
		{
			...
			$this->setdateDebutMesure( $this->list_grilles[$i]['date_debut_mesure'] );
	...
	{
		$this->setText( '{
				"enqueteGrilleId":'.$this->numeroGrille.',
    			"grilleVersionId":3,
